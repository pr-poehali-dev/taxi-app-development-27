...
import FaceMasks, { masks } from "@/components/FaceMasks";
...
export default function Editor({ onNavigate }: EditorProps) {
  ...
  const currentMaskOverlay = masks.find((m) => m.id === activeMask)?.overlay;
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24">
      ...
      <div className="flex items-center justify-between px-4 pt-12 pb-4">
        ...
        <button
          onClick={handleSave}
          disabled={!resultUrl && activeMask === "none"}
      ...
      >
        ...
        {displayUrl ? (
          <>
            ...
            {/* Маска поверх фото */}
            {activeMask !== "none" && !comparing && currentMaskOverlay}
            ...
            <div className="absolute top-3 left-3 flex gap-2">
              ...
              {activeMask !== "none" && (
                <span className="bg-black/60 text-white text-xs px-2 py-1 rounded-full">
                  {masks.find((m) => m.id === activeMask)?.emoji} маска
                </span>
              )}
            </div>
        ...
        <div className="flex bg-card rounded-xl p-1 gap-1">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === "retouch" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === "masks" ? "bg-primary text-primary-foreground" : "text-muted-foreground"
            }`}
          >
            <span className="flex items-center justify-center gap-1.5">
      ...
      {tab === "masks" && (
        <div className="px-4">
          <div className="overflow-x-auto pb-1">
            <FaceMasks activeMask={activeMask} onChange={setActiveMask} />
          </div>
          {activeMask !== "none" && (
            <button
              onClick={() => setActiveMask("none")}
          ...
          {activeMask === "none" && (
            <p className="text-center text-muted-foreground text-sm mt-4">Выбери маску, чтобы наложить её на фото</p>
          )}
        </div>
      )}
    </div>
